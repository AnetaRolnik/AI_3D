function register() {
    ...
    invoiceCheckbox.on("click", function() {
        invoiceCheckbox.prop('checked') ? invoiceData.css('display', 'block') : invoiceData.css('display', 'none');
    });    

    const userCheckbox = $("#user");
    const participantInput = $("#participantsData .required");
    userCheckbox.on("click", function() {
        const nameVal = $("#personalData #name").val();
        if(userCheckbox.prop('checked')) {
            participantInput.val(String(nameVal)).prop('readonly', true).css('background', '#f6f7f6');
        } else { 
